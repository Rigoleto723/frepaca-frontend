...
interface InvestorFormProps {
    investor?: Investor;
    onSave: (investorData: Omit<Investor, 'id' | 'createdAt' | 'updatedAt'>) => void;
    onClose: () => void;
}

const InvestorForm: React.FC<InvestorFormProps> = ({ investor, onSave, onClose }) => {

    const [error, setError] = useState<string | null>(null);
    const [formData, setFormData] = useState<Omit<Investor, 'id' | 'createdAt' | 'updatedAt'>>({
        nombre: investor?.nombre || '',
        apellido: investor?.apellido || '',
        tipoDocumento: investor?.tipoDocumento || 'CC',
        numeroDocumento: investor?.numeroDocumento || '',
        telefono: investor?.telefono || '',
        email: investor?.email || '',
        direccion: investor?.direccion || '',
        ciudad: investor?.ciudad || '',
            ...
            <div className="relative top-20 mx-auto p-5 border w-[800px] shadow-lg rounded-md bg-black">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-medium leading-6 text-white">
                        {investor ? 'Editar Inversor' : 'Nuevo Inversor'}
                    </h3>
                    <Button
                        onClick={onClose}
                        variant={'close'}
